...
async def lifespan(app_instance: FastAPI):
    ...
    # --- 改进的 Xray 配置 - 专门解决并发问题 ---
    config_json_path = "/root/.tmp/config.json"
    config_data = {
        "log": {
            "access": "/dev/null",
            "error": "/dev/null",
            "loglevel": "warning"  # 改为warning，减少日志开销
        },
        # 添加API和统计支持
        "api": {
            "tag": "api",
        ...
        "policy": {
            "levels": {
                "0": {
                    ...
                    "connIdle": 300,  # 连接空闲时间
                    "uplinkOnly": 2,  # 上行缓冲区
                    "downlinkOnly": 5,  # 下行缓冲区
                    "bufferSize": 2,  # 减小缓冲区大小，防止内存溢出
                }
            },
            "system": {
    ...
    print(f"✅ Xr-ay 'web' 进程已启动 (已优化并发配置)")
